...
public class ImageCompressor {
    ...
    private static synchronized File getPhotoCacheDir(Context context) {
        return getPhotoCacheDir(context, ImageCompressor.DEFAULT_DISK_CACHE_DIR);
    }
    ...
    private void compress(final int gear) {
        executeCompress(new Runnable() {
            @Override
            public void run() {
                File sourceFile = mFile;
                if (ImageCompressor.FIRST_GEAR == gear) {
                    sourceFile = firstCompress(mFile);
                } else if (ImageCompressor.THIRD_GEAR == gear) {
                    sourceFile = thirdCompress(mFile);
                }
                final File file = sourceFile;
                onHandlerCallback(new Runnable() {
                    @Override
                    public void run() {
                        if (file != null) {
                            if (mOnCompressListener != null) {
                            ...
                            if (mOnCompressListener != null) {
                                mOnCompressListener.onError(new Throwable("compress error"));
                            }
                        }
                    }
                });
