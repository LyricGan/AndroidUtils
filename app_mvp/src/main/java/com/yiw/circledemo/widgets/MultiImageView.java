...
public class MultiImageView extends LinearLayout {
    private static final int DEFAULT_MAX_PER_ROW_COUNT = 3;// 每行显示最大数
    ...
    private void initView() {
        ...
        if (imagesList.size() == 1) {
            ...
            if (allCount == 4) {
                ...
                maxPerRowCount = DEFAULT_MAX_PER_ROW_COUNT;
            }
            int rowCount = allCount / maxPerRowCount + (allCount % maxPerRowCount > 0 ? 1 : 0);// 行数
            for (int rowCursor = 0; rowCursor < rowCount; rowCursor++) {
                ...
                int columnCount = allCount % maxPerRowCount == 0 ? maxPerRowCount : allCount % maxPerRowCount;//每行的列数
                if (rowCursor != rowCount - 1) {
                    columnCount = maxPerRowCount;
                }
                addView(rowLayout);

                int rowOffset = rowCursor * maxPerRowCount;// 行偏移
