...
public class TitleBar extends RelativeLayout implements AppConstants {
	...
	private void init(Context context) {
		...
		if (screenWidth < 640) {
			rootParams = new LayoutParams(LayoutParams.MATCH_PARENT, 60);
		} else if (screenWidth >= 640 && screenHeight <= 1280) {
			rootParams = new LayoutParams(LayoutParams.MATCH_PARENT, 80);
		} else if (screenWidth >= 1080 && screenHeight <= 1920) {
			rootParams = new LayoutParams(LayoutParams.MATCH_PARENT, 100);
		} else {
			rootParams = new LayoutParams(LayoutParams.MATCH_PARENT, 120);
		}
