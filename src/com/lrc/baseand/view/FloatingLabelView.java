...
public class FloatingLabelView extends LinearLayout implements OnFocusChangeListener {
	...
	private void createLayout(AttributeSet attrs) {
		...
		inpuTextParams = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
		displayTextParams = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
		...
		input.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				if (input.getText().toString().length() > 0 && display.getVisibility() == INVISIBLE) {
					showHint();
				} else if (input.getText().toString().length() == 0 && display.getVisibility() == VISIBLE) {
					hideHint();
				}
			}
