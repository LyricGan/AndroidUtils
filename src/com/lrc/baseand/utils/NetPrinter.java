...
public class NetPrinter {
	...
	public boolean open(String ipaddress, int netport) {
		...
		try {
			SocketAddress socketAddress = new InetSocketAddress(ipaddress, netport);
			mSocket = new Socket();
	...
	public void Set() {
		try {
			mCommand = pcmd.CMD_SetPos();
			OutputStream stream = mSocket.getOutputStream();
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
	...
	public void printText(String pszString, int nFontAlign, int nFontSize,
		...
		try {
			OutputStream stream = mSocket.getOutputStream();
			mCommand = pcmd.CMD_TextAlign(nFontAlign);
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);

			if (ifzhenda == 1) {
				mCommand = pcmd.CMD_FontSize_BTP_M280(nFontSize);
				outbytes = mCommand.getBytes(Charset.forName("ASCII"));
				stream.write(outbytes);

				mCommand = pcmd.CMD_FontSize_BTP_M2801(nFontSize);
				outbytes = mCommand.getBytes(Charset.forName("ASCII"));
				stream.write(outbytes);
			} else {
				mCommand = pcmd.CMD_FontSize(nFontSize);
				outbytes = mCommand.getBytes(Charset.forName("ASCII"));
				stream.write(outbytes);
			}

			mCommand = pszString;// +CMD_Enter();
			outbytes = mCommand.getBytes(Charset.forName("GB2312")); // Charset.defaultCharset());
																	// //forName("UTF-8")
			stream.write(outbytes);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void CutPage(int pagenum) {
		try {
			...
			mCommand = pcmd.CMD_PageGO(pagenum) + pcmd.CMD_Enter();
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);

			mCommand = pcmd.CMD_CutPage() + pcmd.CMD_Enter();
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);
	...
	public void PrintTiaoMa(String numstr, int height, int width, int numweizi,
		...
		try {
			OutputStream stream = mSocket.getOutputStream();
			mCommand = pcmd.CMD_TiaoMaHeight(height);
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);

			mCommand = pcmd.CMD_TiaoMaWidth(width);
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);

			mCommand = pcmd.CMD_TiaoMaWeiZi(numweizi);
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);

			mCommand = pcmd.CMD_TextAlign(fontalign);
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);

			mCommand = pcmd.CMD_FontSize(fontsize);
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);

			mCommand = pcmd.CMD_TiaoMaPrint(numstr);
			outbytes = mCommand.getBytes(Charset.forName("ASCII"));
			stream.write(outbytes);
		} catch (IOException e) {
			e.printStackTrace();
