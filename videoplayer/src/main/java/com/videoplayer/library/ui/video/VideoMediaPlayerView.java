...
public abstract class VideoMediaPlayerView extends RelativeLayout implements IPowerStateListener {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) throws IllegalArgumentException, NullPointerException {
        ...
        int playmode = typedArray.getInt(R.styleable.PlayerView_playmode, MediaPlayMode.PLAY_MODE_FULLSCREEN);
        if (playmode == 0) {
        ...
        this.mMediaPlayerEventActionView.setCallback(new MediaPlayerEventActionView.EventActionViewCallback() {
            ...
            public void onActionReplay() {
                if (hasPlayPermission()) {
                    Log.i(Constants.LOG_TAG, "video event action  view action replay");
            ...
            public void onActionError() {
                if (hasPlayPermission()) {
                    mIsComplete = false;
                    Log.e(Constants.LOG_TAG, "onActionError()---playConfig.getVideoMode():" + playConfig.getVideoMode());
                    switch (playConfig.getVideoMode()) {
                        case PlayConfig.SHORT_VIDEO_MODE:
                            playConfig.setInterruptMode(PlayConfig.INTERRUPT_MODE_PAUSE_RESUME);
                            break;
    ...
    public void onDestroy() {
        ...
        //        mMediaPlayerVideoView.release(true);
        WakeLocker.release();
        //        mNetReceiver.unRegistPhoneBroadCast(getContext());
        ...
        public boolean onInfo(MediaPlayer mp, int what, int extra) {
            switch (what) {
                case MediaPlayer.MEDIA_INFO_METADATA_UPDATE:
                    Log.i(Constants.LOG_TAG, "MEDIA_INFO_METADATA_UPDATE:" + extra);
                    break;
                // 视频缓冲开始
    ...
    protected final IVideoController mMediaPlayerController = new IVideoController() {
        ...
        public void onControllerShow(int playMode) {
            Log.e(Constants.LOG_TAG, "onControllerShow, playMode : " + playMode);
        }

        @Override
        public void onControllerHide(int playMode) {
            Log.e(Constants.LOG_TAG, "onControllerHide, playMode : " + playMode);
        }
