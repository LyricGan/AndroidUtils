...
public class EditPage extends EditPageFakeActivity implements OnClickListener, TextWatcher {
    ...
    private static final int DIM_COLOR = 0x007eff;//0x7f323232;
    ...
    private RelativeLayout getPageView() {
        ...
        rlPage.setBackgroundColor(Color.WHITE);
        if (dialogMode) {
...
//		}
        llTitle.getBtnBack().setImageResource(getBitmapRes(activity, "icon_back"));//修改返回样式
        llTitle.getBtnBack().setOnClickListener(this);
        int resId = getStringRes(activity, "multi_share");
        if (resId > 0) {
            llTitle.getTvTitle().setText(resId);
            llTitle.getTvTitle().setTextColor(Color.BLACK);//文字改为黑色
            llTitle.getTvTitle().setTextSize(17);//字号17
            llTitle.getTvTitle().setTypeface(Typeface.DEFAULT);//默认字体
            llTitle.getTvTitle().setGravity(Gravity.CENTER);//文字居中
        }
        llTitle.getBtnRight().setVisibility(View.VISIBLE);
        resId = getStringRes(activity, "share");
        if (resId > 0) {
            llTitle.getBtnRight().setText(resId);
            llTitle.getBtnRight().setTextColor(Color.BLACK);//文字改为黑色
        ...
        llTitle.getChildAt(1).setVisibility(View.INVISIBLE);//隐藏第一条分隔线
        llTitle.getChildAt(3).setVisibility(View.INVISIBLE);//隐藏第二条分隔线
        return llTitle;
    }
